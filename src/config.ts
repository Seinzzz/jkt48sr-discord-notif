  ...
  message: {
    live: (watcher: WatcherData): EmbedBuilder => {
      ...
      return new EmbedBuilder()
        ...
        .setDescription(
          `Total Gift : \`${giftData?.total ?? 0}\`\n\`\`\`${giftData?.users || "No gift right now"}\`\`\``,
        )
        .setImage(watcher.image)
        .setColor(liveColor)
        .setFooter({
          text: "Showroom Gift Counter",
          iconURL:
            "https://play-lh.googleusercontent.com/AKKnVMfKzyuPK2QLY3WDhhfBKyhW8nq5KjYARLcDgcub4ue5B9RSBzjv4-nU5EA1debu",
        })
